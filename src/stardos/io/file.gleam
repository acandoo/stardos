...
import stardos/io.{type IoError}
...
pub type ExistMode {
  ...
  CreateIfNotExist(
    ...
    behavior: CreationMode,
  )
}

pub type CreationMode {
  Truncate
  Keep
}
...
// ) -> Result(Nil, IoError) {
//   let file_result = open_unsafe(path, permissions, exist_mode)
...
//   open(
//     from: path,
//     given: io.WriteOnly(write_mode),
//     on_exist: CreateIfNotExist(behavior: Truncate),
//     then: callback,
//   )
// }

@external(javascript, "./file_ffi.mjs", "write")
pub fn write(to file: String, with data: BitArray) -> Result(Nil, IoError)
